...
- 创建任务后立即调用 `_launch_task(...)` 投递后台执行，并返回 `task.id`。
...
- `asyncio.create_task(...)` 把 `run_task(...)` 包装成事件循环中的后台协程任务。
...
- `new_version = document.version + 1` 用于重建索引版本隔离。
...
- 当前没有独立 `IndexTaskLauncher`；Python 版本直接在 `IndexService._launch_task(...)` 中使用 `asyncio.create_task(...)`。
...
- 当前 Repository 只 `flush()` 不 `commit()`，保持事务边界由外层 session 管理。

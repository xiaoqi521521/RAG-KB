...
当前 `ChunkRepository.search_by_vector(...)` 和 `search_by_fulltext(...)` 已包含知识库、版本、状态和删除过滤。实施时需要确保所有未来检索方法复用同一约束，而不是新增未过滤的“通用搜索”入口。
...
日志和指标可记录 `user_id`、`kb_id` 数量、操作类型、允许/拒绝结果、权限来源（系统管理员、公开、用户、部门）及耗时。不得记录 JWT、完整问题、chunk 正文、文件内容、完整会话消息或授权密钥。

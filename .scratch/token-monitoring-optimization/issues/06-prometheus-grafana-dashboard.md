...
**Blocked by:** 01 — 建立六类 Token 统计契约与最终回答路径; 02 — 接入 Embedding Token 统计; 03 — 接入 HyDE 与忠实性检测 Token 统计; 04 — 接入 Reranker Token 统计; 05 — 增加全局金额预算闸门与单次异常告警
...
- [ ] 提供总览面板，展示当天金额、5 分钟 Token 速率、估算成本、预算使用比例和预算拒绝次数。
- [ ] 提供按 Token 类型、模型和知识库筛选的速率、小时增量、日增量和阶段成本面板。
- [ ] 提供 usage 缺失、单次超过 `0.01 CNY`、预算 80%/100%和统计 sink 故障状态面板。

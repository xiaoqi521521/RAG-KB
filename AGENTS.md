...
- 当前阶段采用**知识库级逻辑隔离**，不新增独立 `tenant_id`：`kb_id` 是文档与 chunk 的强制数据边界，`department_id` 仅是授权主体属性。相关取舍见 `docs/adr/0003-use-knowledge-base-scoped-logical-isolation.md`。
- 文档、chunk 和索引任务必须通过 `kb_id` 或所属文档关联到知识库；对话会话必须带 `user_id`，消息读取、会话复用和消息写入都必须按会话所有者硬过滤。
- JWT 只承载用户标识；每个新请求必须经身份提供者实时加载当前用户的部门、系统角色和启用状态。不得重新引入固定管理员、JWT 权限缓存或绕过身份提供者的测试分支。
...
- 权限与认证日志不得记录 JWT、完整问题、文档正文、对象路径、会话消息或文档 ID。仅检索层权限过滤的专用审计日志允许记录当前 `user_id` 和被过滤的 `denied_kb_ids`；其他权限与认证日志仍不得记录用户 ID 或知识库 ID。Prometheus Token 指标允许使用当前知识库范围的 `kb_id` 作为受控低基数标签（当前仅 4 个值，多知识库请求使用 `multi`），但严禁使用用户 ID、部门 ID、请求 ID、问题、回答、文档 ID 或对象路径作为标签。
...
- 线上 Token 监控类型固定为 `embedding`、`input`、`answer_generation`、`intent`、`hyde`、`reranker` 和 `faithfulness_check`；`input` 仅统计聊天模型的 System Prompt、用户问题、对话历史和检索上下文输入，其他生成类型只统计对应输出（意图识别分类输出记入 `intent`，追问改写输出仍记入 `hyde`）。评估专用指标中的 RAGAS 判定输入、输出和 Embedding 分别记入 `ragas_input`、`ragas_evaluation` 和 `ragas_embedding`，与评估 RAG 执行阶段的 `input`、`embedding` 区分。
- Token 指标使用现有 Prometheus `/metrics` 出口，标签只包含 `model`、`token_type`、`kb_id`；Grafana 和 Prometheus 查询仅供系统管理员与运维人员使用。
- 线上七类请求 Token 按类型累计到 Redis 用户统计版本，费用在读取时按当前部署单价从 Token 派生、不在 Redis 中存储金额；评估运行消耗写入评估专用 Prometheus 指标并计入全局预算，不写入任何用户的个人累计，也不写入全局每日 Hash。不迁移旧数据，不按模型分桶，不保存请求级明细。
- 每日全局 Token 用量按天分类型累计到 Redis `rag:token:v3:daily:{yyyy}:{mm}:{yyyy-mm-dd}`，只覆盖非评估消耗；该 Hash 不包含 `evaluationTokens`。`/stats/usage/daily` 的 `tokens` 与 `cost` 合并该 Hash 派生结果和数据库 `kb_eval_run_usage` 评估 run 汇总，评估字段单独拆分展示，不重复计数，也不使用 Prometheus 历史查询。
- Grafana 今日 Token 与预算面板读取 `/metrics` 抓取时由 `UsageHistoryService` 聚合的 `rag_daily_usage_tokens` 和 `rag_daily_usage_cost_cny`，口径为 Redis 非评估消耗加数据库评估 run 汇总。Prometheus Counter 仍服务于分来源累计趋势图，不得通过 `increase()` 手工拼出今日预算或全口径 Token 总量。

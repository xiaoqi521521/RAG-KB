...
### 注释与可读性（Code Review 友好）

- **函数/方法级注释（必填）** ：所有新增或修改的函数、方法、核心类，必须添加标准的头部注释（如 Docstring 或 Javadoc）。需简明扼要地说明：该方法的职责、关键入参的含义、返回值的结构，以降低跨文件调用的认知成本。
- **核心逻辑行内注释（必填）** ：对于复杂的业务链路、状态机流转、高阶 API 的非直观用法，必须在代码上方添加具体的行内说明。注释重点解释“为什么这么写（Why）”、“核心边界条件”以及“潜在的踩坑点”。
- **拒绝废话**：严禁翻译代码字面意思（如 `i++ // i加1`），保持代码本身的自文档化（Self-documenting）。
- 参考示例文件 docs\few-shot\代码注释.md
...
- **主动熔断**：排查 Bug 或调试时，若连续 2-3 次尝试未果，必须立即停止，输出当前假设与矛盾点，请求人类介入。
- **规范收尾**：任务完成后，提供符合 Conventional Commits 规范的 Git 提交信息建议，准备交付。

...
class RagQueryServiceV2:
    ...
    async def query(
        ...
        *,
        ...
        logger.info("RAG v2 query started: kb_count=%s", len(kb_ids))

        retrieve_result = await self._retrieve_hits(normalized_question, kb_ids, started_at)
        hits = retrieve_result.hits
        if not hits:
            logger.info("RAG v2 query refused: reason=no_hits kb_count=%s", len(kb_ids))
        ...
        logger.info(
            "RAG v2 query completed: kb_count=%s hit_count=%s latency_ms=%s",
            len(kb_ids),
            len(sources),
            latency_ms,
        )
    ...
    async def _retrieve_hits(
        ...
        try:
            ...
            logger.warning(
                "RAG v2 query embedding failed: elapsed_ms=%s error_type=%s",
            ...
            logger.warning(
                "RAG v2 query embedding dependency failed: elapsed_ms=%s error_type=%s",
                self._elapsed_ms(started_at),
                type(exc).__name__,
            )
            raise HTTPException(
            ...
            logger.exception("RAG v2 retrieval failed")
            raise HTTPException(
    ...
    async def _generate_answer(
        ...
        try:
            ...
            logger.warning(
                "RAG v2 generation failed: elapsed_ms=%s error_type=%s",
        ...
        if not isinstance(content, str) or not content.strip():
            logger.warning("RAG v2 generation returned empty content")
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="生成服务暂时不可用"
            )

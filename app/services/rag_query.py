...
class RagQueryService:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            settings: 应用配置，提供 TopK、阈值和上下文数量限制。
        """
    ...
    async def query(
        ...
        *,
        ...
        if not qualified_hits:
            if hits:
                logger.info(
                    "RAG query refused: reason=low_score user_id=%s kb_ids=%s top_score=%.4f min_score=%.4f",
                    user.user_id,
                    kb_ids,
                    hits[0].score,
                ...
                logger.info("RAG query refused: reason=no_hits user_id=%s kb_ids=%s", user.user_id, kb_ids)
    ...
    async def _embed_question(
        ...
        logger.debug("RAG embedding completed: elapsed_ms=%s", self._elapsed_ms(embedding_started_at))
        return query_vector

    async def _retrieve_hits(self, query_vector: list[float], kb_ids: list[int]):
        ...
        logger.debug("RAG retrieval completed: hit_count=%s elapsed_ms=%s", len(hits), self._elapsed_ms(retrieval_started_at))
        return hits
    ...
    async def _generate_answer(
        ...
        if not isinstance(content, str) or not content.strip():
            logger.warning("RAG generation returned empty content: user_id=%s kb_ids=%s", user.user_id, kb_ids)
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="生成服务暂时不可用")

        logger.debug("RAG generation completed: elapsed_ms=%s", self._elapsed_ms(generation_started_at))
